...
<head>
    ...
    <title>{{ \Modules\Account\Entities\Bill::billNumberFormat($bill->bill_id,$bill->created_by,$bill->workspace)}} | {{ !empty(company_setting('title_text',$bill->created_by,$bill->workspace)) ? company_setting('title_text',$bill->created_by,$bill->workspace) : (!empty(admin_setting('title_text')) ? admin_setting('title_text') :'WorkDo') }}</title>
    <link
        ...
        <div class="invoice-header" style="border-top: 15px solid var(--theme-color);">
            <table>
                <tbody>
                    <tr>
                        <td >
                            <h3 style="text-transform: uppercase; font-size: 40px; font-weight: bold; color: {{$color}};">{{ __('BILL')}}</h3>
                        </td>
                        <td class="text-right">
                        ...
                        <td>
                            <table class="no-space vertical-align-top">
                                <tbody>
                                    <tr>
                                        <td>{{ __('Number: ')}}</td>
                                        <td class="text-right">{{  Modules\Account\Entities\Bill::billNumberFormat($bill->bill_id,$bill->created_by,$bill->workspace)}}</td>
                                    </tr>
                                    <tr>
                                        <td>{{ __('Issue Date:')}}</td>
                                        <td class="text-right">{{company_date_formate($bill->issue_date,$bill->created_by,$bill->workspace)}}</td>
                                    </tr>
                                    <tr>
                                        <td>{{__('Due Date')}}:</td>
                                    ...
                                    @if(!empty($customFields) && count($bill->customField)>0)
                                        @foreach($customFields as $field)
                                            <tr>
                                                <td>{{$field->name}} :</td>
                                                <td class="text-right" style="white-space: normal;"> {{!empty($bill->customField[$field->id])?$bill->customField[$field->id]:'-'}}</td>
                                            </tr>
                                        @endforeach
                                    @endif
                                </tbody>

    ...
    public function create($vendorId)
    {
        if(module_is_active('ProductService'))
        {
            if (Auth::user()->can('bill create'))
            {
                ...
                return view('account::bill.create', compact('vendors', 'bill_number', 'product_services', 'category', 'vendorId','customFields','product_type'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission Denied.'));
        ...
        if(module_is_active('ProductService'))
        {
            if (Auth::user()->can('bill edit'))
            {
                ...
                if ($bill->workspace == getActiveWorkSpace())
                {
                    ...
                    $product_type =\Modules\ProductService\Entities\ProductService::$product_type;
                    return view('account::bill.edit', compact('vendors', 'product_services', 'bill', 'bill_number', 'category','customFields','product_type'));
                    }
                else
                {
                    return redirect()->back()->with('error', __('Permission Denied.'));

...
class VenderController extends Controller
{
    ...
    public function show($e_id)
    {
        if (Auth::user()->can('vendor show'))
        {
            ...
            return view('account::vendor.show', compact('vendor','user','customFields'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function statement(Request $request,$id)
    {
        ...
        $user = \Auth::user();
        $img = get_file(sidebar_logo());

        return view('account::vendor.statement',compact('vendor','vendorDetail','img','user','bill_payment','data'));
    }
